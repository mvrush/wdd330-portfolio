...
searchInput.addEventListener("input", (e) => {
    ...
    users.forEach(user => {
        const isVisible = user.name.toLowerCase().includes(value) || user.url.toLowerCase().includes(value) // simply checks the user typed 'value' created from our event listener and if it matches the user name or email (converted to lower case using 'toLowerCase()'), it puts 'true' in the 'isVisible' const. So as soon as it detects a match, it changes to 'true'.
...
fetch("https://pokeapi.co/api/v2/pokemon?limit=1126") // uses fetch() to call our API
 ...
 // .then(data => console.log("This is the API data we recieved ->", data))  // used to test whether we got our data back.
    ...
    users = data['results'].map(user => { // uses our data object and then pops down to the ['results'] array. Then loops through the data 'results' array (found in our returned object) pulling each 'pokemon' using the 'map()' function and creates a card which it adds to our 'user' array defined above.
    ...
    body.textContent = user.url;  // this looks at our 'user' object and gets the 'email'. It then stores it as 'textContent' and appends that to the 'header' const using the (.)
    ...
    return { name: user.name, url: user.url, element: card} // here we return an object. This object conatins our user name, user email, and the card element.
    })
})   