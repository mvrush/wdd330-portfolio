...
import { makeRequest } from "./authHelpers.js"; // VSCode won't add the .js to your javascript files but without out it you get MIME type errors. Put it on.
// create the 'Auth' class and export it.
export default class Auth {
    ...
    async login(callback) {
        // replace the id's below with whatever you used in your form.
        // Gets the user input from our form and puts it in the 'password' and 'username' const's
      ...
      // you can pass a query to the API by appending it on the end of the url like this: 'users?email=' + email
        ...
        // since we have a token let's go grab some data from the API by executing the callback if one was passed in
            ...
            console.log("error from our login function in the Auth class ->", error);
        }
    }
    ...
    async getCurrentUser(email) {
        try {
            ...
            // if there were any errors display them
            console.log("error from our 'getCurrentUser' function in Auth class ->", error);
        }
    }
